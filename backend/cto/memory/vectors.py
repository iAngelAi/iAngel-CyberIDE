...
class DocumentMetadata(BaseModel):
    ...
    source: str | None = Field(None, max_length=500, description="Document source (file path, URL, etc)")
...
class Document(BaseModel):
    ...
    text: str = Field(..., min_length=1, max_length=100000, description="Document text content")
...
class VectorStore(ABC):
    ...
    @abstractmethod
    async def add(self, id: str, text: str, embedding: list[float], metadata: dict[str, Any]) -> None:
...
class ChromaDBAdapter(VectorStore):
    ...
    def __init__(self, persist_directory: str | Path | None = None):
        ...
        if self.persist_directory:
            ...
            self.client = chromadb.PersistentClient(path=str(self.persist_directory), settings=settings)
        else:
        ...
        logger.info(
            ...
            extra={"persist_directory": str(self.persist_directory) if self.persist_directory else "in-memory"}
        )
    ...
    async def add(self, id: str, text: str, embedding: list[float], metadata: dict[str, Any]) -> None:
        ...
        logger.debug("Document added to ChromaDB", extra={"id": id})
    ...
    async def delete(self, id: str) -> None:
        ...
        logger.debug("Document deleted from ChromaDB", extra={"id": id})
...
class VectorMemory:
    ...
    async def initialize(self) -> None:
        ...
        logger.info("Initializing VectorMemory", extra={"persist_directory": str(self.persist_directory)})
    ...
    async def delete(self, id: str) -> None:
        ...
        await self._store.delete(id)  # type: ignore
        logger.info("Document deleted from vector memory", extra={"id": id})
